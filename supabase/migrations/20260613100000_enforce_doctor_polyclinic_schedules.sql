-- Enforce that doctor schedules use an assigned doctor-polyclinic pair.
-- This keeps the existing queue/session/ticket flow unchanged and only
-- hardens the schedule master-data relationship.
...
on conflict (doctor_id, polyclinic_id) do nothing;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'doctor_schedules_doctor_polyclinic_fk'
      and conrelid = 'public.doctor_schedules'::regclass
  ) then
    alter table public.doctor_schedules
      add constraint doctor_schedules_doctor_polyclinic_fk
      foreign key (doctor_id, polyclinic_id)
      references public.doctor_polyclinics (doctor_id, polyclinic_id)
      on update restrict
      on delete restrict
      not valid;
  end if;
...
alter table public.doctor_schedules
  validate constraint doctor_schedules_doctor_polyclinic_fk;
...
begin
  ...
  if not exists (
    select 1
    from public.doctor_polyclinics
    where doctor_id = p_doctor_id
      and polyclinic_id = p_polyclinic_id
      and is_active = true
  ) then
    raise exception 'Doctor is not assigned to this polyclinic';
  end if;
