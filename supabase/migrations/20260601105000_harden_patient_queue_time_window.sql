-- Make patient queue-taking time aware and expose takeability to the mobile app.
-- Business rule: patients can only take a ticket on the service date, inside
-- the schedule's practice window, while the schedule/session/master data are active.
